...
export const search = async (searchTerm, page = 1, limit = 10) => {
  const { data } = await axios.get(`/api/products/search/${searchTerm}?page=${page}&limit=${limit}`);
...
export const getAllByTag = async (tag, page = 1, limit = 10) => {
  ...
  const { data } = await axios.get(`/api/products/tag/${tag}?page=${page}&limit=${limit}`);
  return data;
};
...
export const getProductRatings = async (productId) => {
  const { data } = await axios.get(`/api/products/${productId}/ratings`);
  return data;
};

// Rate a product
export const rateProduct = async (productId, rating) => {
  const response = await axios.put(`/api/products/rate/${productId}`, { stars: rating });
  return response.data;
};

// Add a comment to product
export const addComment = async (productId, userId, content) => {
  const { data } = await axios.post(`/api/products/${productId}/comments`, { userId, content });
  return data;
};
